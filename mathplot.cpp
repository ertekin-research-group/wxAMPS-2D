...
#ifdef _WINDOWS
		wxClientDC dc(&w);
		wxColour col(_("WHITE"));
		dc.GetPixel(mouseX,mouseY,&col);
		if(col!=*wxWHITE)
			m_content.Printf(wxT("x = %.4f y = %.4g"), w.p2x(mouseX), w.p2y(mouseY));
		else
			m_content.Clear();
#else
...
void mpFXYVector::SetData( const std::vector<double> &xs,const std::vector<double> &ys)
{
    ...
    if (xs.size()>0)
    {
        ...
        m_minX-=0.05*fabs(m_minX);//0.5f;
        m_minY-=0.05*fabs(m_minY);
        m_maxX+=0.05*fabs(m_maxX);//0.5f;
        m_maxY+=0.05*fabs(m_maxY);
    }
    else
    {
        m_minX  = -1;
...
bool mpWindow::CopyScreenshot(int type, wxSize imageSize, bool fit)
{
	int sizeX, sizeY;
	int bk_scrX, bk_scrY;
	if (imageSize == wxDefaultSize) {
	...
	wxBitmapDataObject *bmpObj=new wxBitmapDataObject;
	bmpObj->SetBitmap(screenBuffer);
	if (wxTheClipboard->Open())
	{
		wxTheClipboard->SetData(bmpObj);
